...
class pedido:
    def __init__(self, contacto, cantidadPeces1, cantidadPeces2, totalPagar):
        ...
        pez1.cantidad = pez1.cantidad - int(ListaPedidos[contador][1])
        pez2.cantidad = pez2.cantidad - int(ListaPedidos[contador][2])
        
        cur.execute("INSERT INTO pedidos VALUES(?,?,?,?,?)", (elPedido))
        
        print (ListaPedidos)

        if pez1.cantidad < 0 or pez2.cantidad < 0:
            pedido.cancelar(contador)
        

    def cancelar(idSelec):
        pez1.cantidad = pez1.cantidad + int(ListaPedidos[int(idSelec)][1])
        pez2.cantidad = pez2.cantidad + int(ListaPedidos[int(idSelec)][2])
    ...
    def modificarPedido(idSelec, cantidadPeces1, cantidadPeces2, contacto, presioR1, presioR2):
        pez1.cantidad = pez1.cantidad + int(ListaPedidos[int(idSelec)][1]) - int(cantidadPeces1)
        pez2.cantidad = pez2.cantidad + int(ListaPedidos[int(idSelec)][2]) - int(cantidadPeces2)
